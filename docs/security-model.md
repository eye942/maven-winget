...
Actions use read-only defaults and are pinned where practical. Each GitHub-hosted workflow job begins with StepSecurity Harden-Runner in audit mode to record unexpected outbound traffic without blocking builds; it is intentionally pinned to a commit SHA. Pull requests do not receive secrets. Release permissions are narrowly scoped to contents write and can later call a signing service through protected secrets without changing MSI structure. No signing key is committed; absent Authenticode signing is disclosed in releases.
