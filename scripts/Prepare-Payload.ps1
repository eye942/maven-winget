...
$wix=Get-Command wix -ErrorAction Stop; $fragment=Join-Path $OutputDirectory 'Components.wxs'; & $wix.Path heat dir $root -dr INSTALLDIR -cg MavenPayload -gg -srd -sfrag -out $fragment; if($LASTEXITCODE){throw 'WiX Heat failed.'}
